...
**@openacp/workspace-plugin** is an OpenACP plugin.
...
async setup(ctx: PluginContext) {
  ctx.registerMiddleware('message:outgoing', {
    ...
    handler: async (payload, next) => {
      ...
      payload.message.text += '\n-- sent via @openacp/workspace-plugin'
...
async setup(ctx: PluginContext) {
  ...
  if (!apiKey) {
    ctx.log.warn('Not configured — run: openacp plugin configure @openacp/workspace-plugin')
    return
  }
  // Use apiKey...
}
...
describe('@openacp/workspace-plugin', () => {
  it('registers commands on setup', async () => {
    const ctx = createTestContext({ pluginName: '@openacp/workspace-plugin' })
...
4. Users install with:
   ```bash
   openacp plugin install @openacp/workspace-plugin
   ```
