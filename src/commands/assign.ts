...
export function makeAssignCommand(
  ...
  return {
    ...
    async handler(args) {
      ...
      const [assigneeUserId] = await resolveMentions(mentions, identity)
      if (!assigneeUserId) return { type: 'error', message: `User @${mentions[0]} not found. They need to send a message first.` }

      const isParticipant = session.participants.some(p => p.userId === assigneeUserId)
      if (!isParticipant) {
        const user = await identity.getUser(assigneeUserId)
        return { type: 'error', message: `${user?.displayName ?? assigneeUserId} is not a participant in this session.` }
      }

      const title = args.raw.replace(/@[a-zA-Z0-9_.-]+/g, '').trim()
      if (!title) return { type: 'error', message: 'Please provide a task description.' }

      const taskId = await store.addTask(title, assigneeUserId)
      const user = await identity.getUser(assigneeUserId)

      await ctx.emitHook('taskAssigned', { sessionId: args.sessionId, taskId, assignee: assigneeUserId, title })

      return { type: 'text', text: `✅ Task assigned to ${user?.displayName ?? assigneeUserId}: "${title}" (${taskId})` }
    },
  }
}
